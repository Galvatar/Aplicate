...
export default function UploadSection({ userId }: { userId: string }) {
    ...
    const csvPrompt = `
    Transform the following into a single downloadable CSV file (no need for more than one option) such that it has the following columns:
    title, company, employmentType, foundOn, status, location, applied, lastUpdate, journey, notes, pay, url, mainContact, rating, closingDate, followUpDate
    ...
    Applied, lastUpdate, closingDate, followUpDate are all dates.
    ...
    All the dates must be formatted as iso 8601 strings if it contains any.
        ...
        <div className="flex flex-col gap-3">
            ...
            >
                ...
                <button
                    ...
                    className="flex gap-2 px-5 py-3 mt-5 bg-primary hover:bg-primary/50 text-on-primary font-bold rounded-xl transition-colors">
